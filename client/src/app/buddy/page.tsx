...
export default function BuddyPage() {
  ...
  useLayoutEffect(() => {
    if (!submitted) return;
    const form = selectedType === "STUDENT" ? "student" : "mentor";
    window.history.replaceState(null, "", `/buddy?form=${form}`);
    window.scrollTo(0, 0);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(snap, 50);
    return () => {
  ...
  return (
    <Layout>
      <div dir={isRtl ? "rtl" : "ltr"} className="space-y-12 pb-12 pt-4 sm:space-y-16 sm:pt-8" style={{ overflowAnchor: "none" }}>
        ...
        <section ref={formRef} id="application" tabIndex={-1} className="scroll-mt-20 rounded-3xl bg-white p-5 shadow-xl ring-1 ring-slate-200 sm:scroll-mt-24 sm:p-8 lg:p-10">
          <div className="mx-auto max-w-3xl">
            ...
            {authLoading ? <div className="flex justify-center py-14"><Loader2 className="h-7 w-7 animate-spin text-indigo-600" /></div> : !user ? (
              ...
              <div data-testid="buddy-success" tabIndex={-1} className="mt-6 flex min-h-[240px] flex-col items-center justify-center rounded-2xl bg-emerald-50 px-5 py-8 text-center ring-1 ring-emerald-100 outline-none sm:min-h-[280px] sm:p-10">
                <CheckCircle2 className="h-10 w-10 text-emerald-600" />
                <h3 className="mt-3 text-xl font-bold text-slate-900">{bt("success.title")}</h3>
                <p className="mt-2 max-w-md text-sm leading-6 text-slate-600">{bt("success.desc")}</p>
                <Button variant="outline" onClick={() => { setSubmitted(null); requestAnimationFrame(() => formRef.current?.scrollIntoView({ block: "start", behavior: "auto" })); }} className="mt-5">{bt("success.another")}</Button>
              </div>
        ...
        {user && (
          <section aria-labelledby="buddy-mine-heading" className="rounded-3xl bg-blue-50 p-5 ring-1 ring-blue-100 sm:p-7" style={{ overflowAnchor: "none" }}>
            ...
            {mineLoading ? <Loader2 className="mt-5 h-5 w-5 animate-spin text-blue-600" /> : mineError ? <p role="alert" className="mt-4 text-sm text-red-700">{mineError}</p> : applications.length === 0 ? <p className="mt-4 text-sm text-slate-600">{bt("mine.empty")}</p> : (
              <div className="mt-4">
                <BuddyMineCards applications={applications} locale={localeFor(currentLanguage)} bt={bt} />
              </div>
            )}
          </section>
        )}
