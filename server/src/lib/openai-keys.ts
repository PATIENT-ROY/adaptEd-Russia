/**
 * Утилита для управления и ротации OpenAI API ключей
 */
...
export function getNextApiKey(): string | null {
  ...
  if (failedKeys.size >= OPENAI_API_KEYS.length) {
...
export function markKeyAsFailed(key: string): void {
  failedKeys.add(key);
  console.warn(`API ключ помечен как нерабочий: ${key.substring(0, 10)}...`);
}
...
export function resetKey(key: string): void {
  ...
  console.log(`API ключ сброшен и снова доступен: ${key.substring(0, 10)}...`);
}
...
export function getAvailableKeysCount(): number {
  return OPENAI_API_KEYS.length - failedKeys.size;
}
